...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            if params['do_dagger']:
                ...
                for i, path in enumerate(paths):
                    obs_np = path["observation"]
                    expert_acs = expert_policy.get_action(obs_np)
                    paths[i]["action"] = expert_acs
        ...
        for _ in range(params['num_agent_train_steps_per_iter']):
          ...
          inds = np.random.permutation(len(replay_buffer.obs))[:params['train_batch_size']]
          ob_batch, ac_batch = replay_buffer.obs[inds], replay_buffer.acs[inds] 
